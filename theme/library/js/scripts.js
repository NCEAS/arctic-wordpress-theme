...
function slaaskCustomization() {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      $(
        "#slaask-widget-container .conversation-text p:contains('" +
          slaaskOptions.closeMsg +
          "')"
      )
        .parents('.conversation-text')
        .hide();
      $(
        "#slaask-widget-container .conversation-text p:contains('" +
          slaaskOptions.openMsg +
          "')"
      )
        .parents('.conversation-text')
        .hide();
  ...
  function isChatting() {
    ...
    //If there is only one message in the widget, and its from us, then no
      ...
      messages.length == 1 &&
      messages.first().find('.chat-avatar').length > 1
    )
      return false;
    //If the conversation has been marked as closed, then no
      ...
      messages
        .last()
        .find('.slaask-notification')
        .text()
        .indexOf('marked as closed') > -1
    )
      return false;
    else return true;
  }
